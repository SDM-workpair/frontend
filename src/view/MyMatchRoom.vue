<template>
    ...
    <v-app>
        ...
        <v-main>
          ...
          <h2 class="text-left hahaMR">&nbsp&nbsp&nbsp&nbsp{{$t('my_matching_space')}}</h2>
          <hr class="h-color mx-2">
          <v-container fluid>
            
            <v-row dense>
              <v-col
              v-for="(item, index) in matching_rooms" :key="index"
              >
                <v-expand-transition>
                  <v-card
                      class="mx-auto"
  ...
  refresh_mr(){
    ...
    .then( (mr_response) => {this.matching_rooms = mr_response.data.data.map
        (room =>{return{
          name: room.name,
